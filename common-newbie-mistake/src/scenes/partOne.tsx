...
import { easeInOutCubic, easeOutCubic, tween } from "@motion-canvas/core/lib/tweening";
...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <>
      <ChapterTitle ref={chapterTitle} chapterNumberText={"1"} chapterTitleText={"MENGHAFAL\nSYNTAX"} />
      <JetbrainText x={() => examplePosition() * -1100} y={-400} text={"Dibawah merupakan kode\nuntuk menampilkan text dari 0-9:"} ref={questionHeader} />
      <CodeBlock ref={exampleCode} scale={1} y={-150} x={() => examplePosition() * -999} fontSize={90} language="py" code={`for i in range(10):\n\t\t\t\tprint(i)`} />
      <Image src={oldDude} height={600} y={150} opacity={0} ref={exampleImage} />
      <Line lineWidth={10} stroke={"#fff"} ref={highlightLine}
      y={-170}
      closed
      points={[
        [0, 0],
      ...
      <Rect width={100} height={100} scale={0} radius={100} fill={"#2e7d32"} ref={skillBlock} />
      <Rect width={100} height={100} scale={0} radius={100} fill={"#d84315"} ref={skillBlock2} />
      <Rect width={100} height={100} scale={0} radius={100} fill={"#1565c0"} ref={skillBlock3} />
      <JetbrainText x={-50} y={510} opacity={0} text={"MENAMPILKAN\n\t\t\tTEXT"} ref={skillText1} />
      <JetbrainText x={300} y={500} opacity={0} text={"PERULANGAN"} ref={skillText2} />
      <JetbrainText x={0} y={800} opacity={0} text={"MEMBATASI\nPERULANGAN"} ref={skillText3} />

      <JetbrainText y={-400} x={-1100} scale={0.9} ref={anotherQuestion} text={"BUATLAH SEBUAH KODE YANG MENAMPILKAN\nNAMA ANDA 10 KALI"} />
      <Circle width={100} height={100} scale={0} fill={"3a3a3a"} ref={finalAnswer} />
      <CodeBlock ref={finalAnswerCode} language="py" scale={0} code={`for i in range(10):\n\t\t\tprint("Reeyo")`} />
    </>
  );
  ...
  yield* all(
    ...
    delay(useDuration("D2_3"),
    exampleCode().selection(word(0, 0, 20), 0.5)
    ),
    delay(useDuration("D3"), skillBlock2().position.x(easeOutCubic(1, 0, 300), 1)),
    delay(useDuration("D3_1"), skillBlock2().position.y(easeOutCubic(1, 0, 500), 1)),
    delay(useDuration("D3_2"), skillBlock2().radius(10, 1)),
    delay(useDuration("D3_3"), skillBlock2().width(200, 1)),
    delay(useDuration("D3_4"), 
    tween(0.5, value => {
    ...
    delay(useDuration("D4_2"),
    exampleCode().selection(word(0, 9, 20), 0.5),
    ),
    delay(useDuration("D4_3"), skillBlock3().position.x(easeOutCubic(1, 0, 0), 1)),
    delay(useDuration("D4_4"), skillBlock3().position.y(easeOutCubic(1, 0, 800), 1)),
    delay(useDuration("D4_5"), skillBlock3().radius(10, 1)),
    delay(useDuration("D4_6"), skillBlock3().width(200, 1)),
    delay(useDuration("D4_7"), 
    tween(0.5, value => {
  ...
  yield* all(
    ...
    tween(1, value => {
      highlightLine().position.x(easeInOutCubic(value, highlightLine().position.x(), 1100))
    }),
  );
  yield* tween(1, value => {
...
function* combineIt(skillText: Reference<JetbrainText>, skillBlock: Reference<Rect>): ThreadGenerator {
  yield* all(
    ...
    tween(1, value => {
      skillBlock().position.x(easeInOutCubic(value, skillBlock().position.x(), 0))
    }),
    tween(1, value => {
      skillBlock().position.y(easeInOutCubic(value, skillBlock().position.y(), 0))
    }),
    delay(0.8, tween(1, value => {
